...
def message_react_v1(token, message_id, react_id):
    ...
    return requests.post(f'{url}message/react/v1', json = pload)
...
# Standard Case - Channels
def test_messages_react_standard_ch(setup):
    user1 = setup[0]
    user2 = setup[1]
    ch = setup[2]
    ch_msg1 = setup[3]
    ...
    response = message_react_v1(user1['token'], ch_msg1, 1)
    assert response.status_code == Success_code
    response = message_react_v1(user1['token'], ch_msg2, 1)
    assert response.status_code == Success_code
    response = message_react_v1(user2['token'], ch_msg1, 1)
    assert response.status_code == Success_code
...
# Standard Case - Dms
def test_messages_react_standard_dm(setup):
    user1 = setup[0]
    user2 = setup[1]
    dm = setup[5]
    dm_msg1 = setup[6]
    ...
    response = message_react_v1(user1['token'], dm_msg1, 1)
    assert response.status_code == Success_code
    response = message_react_v1(user1['token'], dm_msg2, 1)
    assert response.status_code == Success_code
    response = message_react_v1(user2['token'], dm_msg1, 1)
    assert response.status_code == Success_code
...
# Invalid token
def test_messages_react_invalid_token(setup):
    ch_msg1 = setup[3]
    
    response = message_react_v1('not_a_token', ch_msg1, 1)
    assert response.status_code == AccessError.code

# User not in msg ch
def test_messages_react_user_not_in_ch(setup):
    user3 = auth_register_v2('biology@gmail.com', 'testtest', 'Tae', 'Pham')
    ch_msg1 = setup[3]

    response = message_react_v1(user3['token'], ch_msg1, 1)
    assert response.status_code == InputError.code

# Invalid message_id
def test_messages_react_invalid_msg_id(setup):
    user1 = setup[0]

    not_msg_id = 20
    response = message_react_v1(user1['token'], not_msg_id, 1)
    assert response.status_code == InputError.code

# Already has react_id
def test_messages_react_existing_react_id(setup):
    user1 = setup[0]
    dm_msg1 = setup[6]
...
# Invalid react_id
def test_messages_react_invalid_react_id(setup):
    user1 = setup[0]
    ch_msg1 = setup[3]

    invalid_react_id = -11
    response = message_react_v1(user1['token'], ch_msg1, invalid_react_id)
    assert response.status_code == InputError.code