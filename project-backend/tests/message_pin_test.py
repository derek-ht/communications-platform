...
# Standard Case - Chs
def test_message_pin_standard_ch(setup):
    user1 = setup[0]
    user2 = setup[1]
    ch = setup[2]
    ch_msg1 = setup[3]
    ...
    response = message_pin_v1(user1['token'], ch_msg1)
    assert response.status_code == Success_code
    response = message_pin_v1(user1['token'], ch_msg2)
    assert response.status_code == Success_code
...
# Standard Case - Dms
def test_message_pin_standard_dm(setup):
    user1 = setup[0]
    user2 = setup[1]
    dm = setup[6]
    dm_msg1 = setup[7]
    ...
    response = message_pin_v1(user2['token'], dm_msg1)
    assert response.status_code == Success_code
    response = message_pin_v1(user2['token'], dm_msg3)
    assert response.status_code == Success_code
...
def test_message_pin_invalid_token(setup):
    ...
    response = message_pin_v1('not_a_token', ch_msg1)
    assert response.status_code == AccessError.code

# User does not have owner perms
def test_message_pin_user_no_perms(setup):
    user1 = setup[0]
    user2 = setup[1]
    ch_msg1 = setup[3]
    dm_msg1 = setup[7]

    response = message_pin_v1(user1['token'], dm_msg1)
    assert response.status_code == AccessError.code
    response = message_pin_v1(user2['token'], ch_msg1)
    assert response.status_code == AccessError.code

# User not in the Ch
def test_message_pin_user_not_in_ch(setup):
    user3 = auth_register_v2('biology@gmail.com', 'testtest', 'Tae', 'Pham')
    ch_msg1 = setup[3]

    response = message_pin_v1(user3['token'], ch_msg1)
    assert response.status_code == InputError.code

# Invalid message Id
def test_message_invalid_message_id(setup):
    user1 = setup[0]

    not_msg_id = 20
    response = message_pin_v1(user1['token'], not_msg_id)
    assert response.status_code == InputError.code

# Message already pinned
def test_message_already_pinned(setup):
    user1 = setup[0]
    user2 = setup[1]
    ch_msg1 = setup[3]
    dm_msg1 = setup[7]
